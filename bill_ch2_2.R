...
#fig2
#match iso
#merge agrifood_fg_lu and ppp for 90-20
f_raw1<-read_xlsx("C:/Users/wb565654/OneDrive - WBG/project/DARL/data/newfao/newdataset20162020/agrifood_fu_lu9020/agrifood_fu_lu9020.xlsx",
                  sheet="Sheet1")
f_raw2<-read_xlsx("C:/Users/wb565654/OneDrive - WBG/project/DARL/data/newfao/newdataset20162020/agrifood_fu_lu9020/agrifood_ppp9020.xlsx",
                  sheet="Sheet1")
f_raw<-rbind(f_raw1, f_raw2)

#############################################################################################################################################
######################now modify items
####merge Agrifood Systems Waste Disposal
waste<-subset(f_raw,f_raw$Item=="Domestic Wastewater" | f_raw$Item=="Incineration" | f_raw$Item=="Industrial Wastewater" | f_raw$Item=="Solid Food Waste")
waste<-waste%>% 
  group_by(ISO, Area, Year) %>% 
  summarise(Value=sum(Value)) 
waste$Item <-"Agrifood Systems Waste Disposal"
waste<-as.data.frame(waste)
f_raw<-subset(f_raw, f_raw$Item!="Domestic Wastewater" & f_raw$Item!="Incineration" & f_raw$Item!="Industrial Wastewater" & f_raw$Item!="Solid Food Waste")
f_raw<-rbind(f_raw, waste)

f_raw<-subset(f_raw, f_raw$Item!="Drained organic soils (CO2)" &f_raw$Item!="Drained organic soils (N2O)")

f_raw<-subset(f_raw, f_raw$Item!="Pre- and Post- Production" & f_raw$Item!="Land Use change"& f_raw$Item!="Farm gate")
...
country <- country[!is.na(country$`Income group`),]

f_c<-merge(x=f_raw, y=country, by.x="ISO", by.y="Code", all.x=F)

f_c$`Income group` <- factor(f_c$`Income group`, level=c("Low income","Middle income","High income" ))

f_c$Value<-f_c$Value/1000000
f_c2<-f_c %>% 
...
f_c2$`Income group`[is.na(f_c2$`Income group`)] <- 'Global total' #replace NA with text

a_2$`Income group` <- factor(a_2$`Income group`,                 # Relevel group factor
                           levels = c("Global total", "High income", "Middle income","Low income"))
pf_c2 <- f_c2 %>% #line chart
  ggplot( aes(Year, Value, color=`Income group`)) + labs(y= "GtCO2eq") +
...
pf_c2<-ggplot(data=f_c2, aes(x=Year, y=Value, fill = `Income group`)) +
  # geom_col(aes(fill = Region), position = "dodge") +
  geom_area(stat="identity")+theme_bw()+labs(y=(bquote('Gt'~CO[2]~'eq')))
pf_c2

write.csv(f_c2,"C:/Users/wb565654/OneDrive - WBG/project/DARL/data/request_authors/bill/chapter2/fig2/Agri-food system emissions by income groups over time.csv")
