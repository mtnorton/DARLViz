...
roe_pc$GEO_AREA_NAME <- factor(roe_pc$variable,                 # Relevel group factor
                          levels = c("totag_re_feas", "totag_sc_feas", "totdem_feas", "formgmts_feas", "defors_feas","ars_feas"))
...
########################################050123
#Agriculture Orientation Index, by SDG region 
ag_raw<-read_xlsx('C:/Users/wb565654/OneDrive - WBG/project/DARL/data/request_authors/ashesh/Ag_Orientation_Public_Expenditure_FAOSTAT.xlsx',
              sheet="Sheet2")

colnames(ag_raw)[2]<-"Year"
ag_raw$Year<-as.character(ag_raw$Year)
ag_raw$Year<-factor(ag_raw$Year, levels = c("2016","2021"))

ag_raw$GEO_AREA_NAME <- factor(ag_raw$GEO_AREA_NAME,                 # Relevel group factor
                               levels = c("Oceania, excl Australia & New Zealand","Sub-Saharan Africa","Latin America & the Caribbean",
                                          "Australia & New Zealand","Northern America & Europe","Western Asia","Northern Africa",
                                          "Central Asia & Southern Asia","Eastern Asia & South-eastern Asia","World" ))

pag<-ggplot(data=ag_raw, aes(x=GEO_AREA_NAME, y=OBS_VALUE, fill = Year)) +
  # geom_col(aes(fill = Region), position = "dodge") +
  geom_bar(stat="identity", position = "dodge")+ #position dodge means two year's bar side by side
  scale_fill_discrete(breaks=c('2021',"2016" )) #order in the label
#scale_fill_brewer() #this automatically assign color
#geom_text(aes(label = freq), color="#dff6f8", position = position_stack(vjust = 0.5)) #add color for cost effective
pag
pag +xlab("")+ylab("")+ theme_bw()+ coord_flip() #rotate x and y axis

pag<-pag+coord_flip()+ xlab("")+ylab("")+ggtitle("                     Agriculture Orientation Index, by SDG region ")+ theme_bw()
pag
